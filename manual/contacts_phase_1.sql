...
-- Insert meta records
    ...
    SELECT 'contacts_linked_to_job', cname FROM #p7m_meta
    UNION ALL
...
SET sql_where = '
WHERE pos.contact_status IS NOT NULL 
...
GROUP BY
  ...
  ,[Notes]';
    ...
    EXECUTE (REPLACE(sql_start, '#table', '#p7m_contacts_phase_1') || sql_where || sql_end );
...
WHERE pos.contact_status IS NOT NULL
  ...
  AND (EXISTS (SELECT 1
              ...
              WHERE er.person_ref IS NOT NULL
                ...
                AND er.person_ref = per.person_ref
              )
       OR
       EXISTS (SELECT 1
               FROM opportunity o
               ...
               WHERE r.person_ref IS NOT NULL
                AND EXISTS (SELECT 1
                 ...
                 AND o.responsible_team IN(SELECT [value]
                                           FROM #p7m_vars WHERE [key] = ''teams'')
                 AND o.create_timestamp >= CAST(''2012-10-01'' AS DATETIME)
                 AND r.person_ref = per.person_ref
              )
       )
  AND per.person_ref NOT IN(SELECT ref FROM #p7m_contacts_phase_1)';
    
    EXECUTE (REPLACE(sql_start, '#table', '#p7m_contacts_link_to_job') || sql_where || sql_end );
...
WHERE pos.contact_status IS NOT NULL
  ...
  AND per.person_ref NOT IN(SELECT ref FROM #p7m_contacts_phase_1)
  AND per.person_ref NOT IN(SELECT ref FROM #p7m_contacts_link_to_job)';
  
    EXECUTE (REPLACE(sql_start, '#table', '#p7m_contacts_client_visit') || sql_where || sql_end );
...
WHERE pos.contact_status IS NOT NULL
  ...
  AND per.person_ref NOT IN(SELECT ref FROM #p7m_contacts_phase_1)
  AND per.person_ref NOT IN(SELECT ref FROM #p7m_contacts_link_to_job)
  AND per.person_ref NOT IN(SELECT ref FROM #p7m_contacts_client_visit)';
   
    EXECUTE (REPLACE(sql_start, '#table', '#p7m_contacts_offer') || sql_where || sql_end );
