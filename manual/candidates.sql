...
-- Get valid Candidates
    ...
    INTO #valid_candidates
    FROM person per
    WHERE EXISTS (SELECT 1 FROM person_type pt
                  WHERE per.person_ref = pt.person_ref 
                    AND pt.type IN('A', 'C'));

    UPDATE #valid_candidates 
    SET extract_type = 'candidates_off_2y'
    FROM #valid_candidates vc
    WHERE EXISTS (SELECT 1 FROM event_role er
    ...
    UPDATE #valid_candidates 
    SET extract_type = 'candidates_reg_int'
    FROM #valid_candidates vc
    WHERE EXISTS (SELECT 1 FROM event_role er
                    INNER JOIN event e ON er.event_ref = e.event_ref
    ...
    UPDATE #valid_candidates 
    SET extract_type = 'candidates_phase_1'
    FROM #valid_candidates vc
...
SELECT 
  ...
  ,LIST(CASE WHEN code_type IN(1008, 1036, 1039, 1041, 1030, 
                               1007, 1010, 1011, 1012, 1014, 1016, 1017, 1018, 1055) 
...
FROM person per
  ...
  LEFT OUTER JOIN #p7m_vars teams
               ON per.responsible_team = teams.[value] AND teams.[key] = ''teams''
WHERE EXISTS (SELECT 1
...
GROUP BY
  ...
  ,date_of_birth';
    ...
    EXECUTE (REPLACE(sql_cmd, '#table', 'candidates_phase_1') );

    EXECUTE (REPLACE(sql_cmd, '#table', 'candidates_reg_int') );

    EXECUTE (REPLACE(sql_cmd, '#table', 'candidates_off_2y') );
