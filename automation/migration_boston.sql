...
-- Load meta data
    ...
    INSERT INTO #p7m_meta VALUES('internal_interview_arranged_journals',
      'journal_id,datetime,consultant,candidate,contact,client,job,notes');
    ...
    UPDATE #p7m_meta
    SET cname = REPLACE(cname, ',accepted_by,acc_off_date,acc_off_time', '');
...
-- Load data for SHORTLIST.CSV
    ...
    SELECT
      ...
      ,opp.organisation_ref AS client_id
      ,can.person_ref AS candidate_id
      ,MAX(con.person_ref) AS person_id
      ...
      ,MAX(CASE WHEN e.event_ref = ISNULL(last_offer_event_ref, last_event_ref)
      ...
      ,MIN(CASE e.event_ref WHEN last_offer_event_ref
                            THEN ISNULL(tb.start_date, pla.start_date) END) AS [last_of_dt]
      ,NULL AS [last_of_tm]
      ...
      ,MIN(CASE e.event_ref WHEN last_rejected_offer_event_ref
                            THEN CAST(e.update_timestamp AS TIME) END) AS [rej_of_tm]
      ...
      ,MIN(CASE e.event_ref WHEN last_rejected_event_ref
                            THEN CAST(e.update_timestamp AS TIME) END) AS [rej_tm]
...
-- Load data for JOURNALS
    ...
    SELECT
      ...
      ,NULLIF(e.organisation_ref, 0) AS [client]
    ...
    CALL write_csv('general_journals', csv_file_path);
